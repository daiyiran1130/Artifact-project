"""
Color fundus image classification using MedGemma1.5-4b-it loaded directly via modelscope/transformers.
No Ollama required. The model runs in the current Python process on local GPU/CPU.

Categories: Normal, Diabetic Retinopathy, Age-related Macular Degeneration, Glaucoma
...
# ── 全局配置 ────────────────────────────────────────────────────────────────
IMAGE_DIR   = Path("/root/autodl-tmp/artifact/fundus/nolabel")
...
def query_model(image_path: Path) -> str:
    """
    用已加载的模型对单张眼底图像做推理，返回模型输出的原始文本。

    直接用 PIL.Image.open() 读图，通过 processor.apply_chat_template()
    构造模型输入张量，在同一 Python 进程内完成推理。
    """
...
def main():
    ...
    print(f"Found {len(numbered)} images. Starting fundus classification...\n")
